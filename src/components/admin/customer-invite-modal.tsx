...
export function inviteMessage({
  ...
  return [
    ...
    `מספר הטלפון להתחברות: ${phone}`,
    "",
    "בכניסה הזינו את המספר הזה ותקבלו קוד אימות חד-פעמי ב-SMS — אין צורך בסיסמה.",
    ...
    <Modal open={open} title="שליחת פרטי התחברות ללקוח" xClose onClose={onClose}>
      <div className="flex flex-col gap-3">
        <div className="text-[12.5px] leading-relaxed text-muted-foreground">
          הלקוח נשמר. אפשר לשלוח לו עכשיו בוואטסאפ קישור לאפליקציה ומספר הטלפון להתחברות —
          הכניסה היא בקוד חד-פעמי ב-SMS, בלי סיסמה.
        </div>
        ...
        <div className="flex gap-2.5">
          <Button variant="secondary" className="flex-1 md:flex-initial md:w-auto font-semibold py-2.5" onClick={copy}>
            <Copy className="size-4" />
            העתקה
          </Button>
          <Button
            className="flex-1 md:flex-initial md:w-auto gap-2 bg-[#25D366] hover:bg-[#1ebe57] text-white py-2.5"
            onClick={send}
          >
